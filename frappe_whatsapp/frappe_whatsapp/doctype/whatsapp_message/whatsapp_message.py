            ...
            if self.content_type in ["document", "image", "video"]:
                ...
                flow_data = {
                    "department": json.dumps([
                        {"id": "dressing", "title": "العناية بالجروح - Dressing"},
                        {"id": "medical_consultations", "title": "الزيارة المنزلية - Medical Consultations"},
                        {"id": "nurse_visit", "title": "الرعاية التمريضية - NURSE VISIT"}
                    ]),
                    "location": json.dumps([
                        {"id": "1", "title": "Jeddah Zone 1"},
                        {"id": "2", "title": "Jeddah Zone 2"}
                    ]),
                    "date": json.dumps([
                        {"id": "2026-02-01", "title": "Mon Feb 01 2026"}
                    ]),
                    "time": json.dumps([
            ...
            if reply_to:
                ...
                if appointment:
                    ...
                    if self.get('message') == "Confirm":
                        ...
                        whatsapp_message.set('message_type', 'Manual')
                        whatsapp_message.set('message',
                                             "To reschedule your appointment, please contact clinic customer service team for assistance")
                        whatsapp_message.set('reply_to_message_id', self.get('message_id'))
                        whatsapp_message.set('content_type', 'text')
                        whatsapp_message.set('whatsapp_account',
                                             frappe.get_single('WhatsApp Settings').get('default_outgoing_account'))
